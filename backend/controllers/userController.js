...
const loginUser = async (req, res) => {
  try {
    ...
    if (isMatch) {
      const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, {
        expiresIn: "30d",
      }); // Optionally add an expiration time to the token
...
const bookAppointment = async (req, res) => {
  try {
    ...
    // Fetch the doctor data and check availability
    ...
    if (slots_booked[slotDate]) {
      ...
      slots_booked[slotDate] = [slotTime]; // Initialize the date with the slot
    }
