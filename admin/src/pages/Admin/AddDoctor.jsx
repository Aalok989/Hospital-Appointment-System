...
const AddDoctor = () => {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
        if (!docImg) {
            return toast.error("Image not Provided");
        }
        ...
        // Sending the token in Authorization header
        const { data } = await axios.post(backendUrl + '/api/admin/add-doctor', formData, {
            headers: {
                Authorization: `Bearer ${aToken}`  // Correctly sending the token
            }
        });
      ...
      <div className="bg-white px-8 py-8 border rounded w-full max-w-4xl max-h-[80vh] overflow-y-scroll">
        <div className="flex items-center gap-4 mb-8 text-gray-500">
          <label htmlFor="doc-img">
            <img className="w-16 bg-gray-100 rounded-full cursor-pointer" src={docImg ? URL.createObjectURL(docImg) : uploadArea} alt="" />
          </label>
